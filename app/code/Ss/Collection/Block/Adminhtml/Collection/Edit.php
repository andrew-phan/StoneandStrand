...
class Edit extends \Magento\Backend\Block\Widget\Form\Container
{
    ...
    {
        $this->_objectId = 'collection_id';
        $this->_blockGroup = 'Ss_Collection';
        $this->_controller = 'adminhtml_collection';

        parent::_construct();

        $this->buttonList->update('save', 'label', __('Save Collection'));
        $this->buttonList->update('delete', 'label', __('Delete'));

        if ($this->getRequest()->getParam('current_collection_id')) {
            ...
            if ($bannerId = $this->getRequest()->getParam('collection_id')) {
                $this->_formScripts[] = '
					window.collection_id = ' . $bannerId . ';
				';
            }
        } else {
            $this->buttonList->add(
    ...
    protected function getSaveAndContinueUrl()
    {
        return $this->getUrl(
                ...
                'current_collection_id' => $this->getRequest()->getParam('current_collection_id'),
                ]
        );
    }
