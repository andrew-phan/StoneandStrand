...
class InputTable extends \Magento\Backend\Block\Widget\Grid\Extended//\Magento\Backend\Block\Widget\Grid
    ...
    public function addColumn($columnId, $column)
    {
        if (is_array($column)) {
            $this->getColumnSet()->setChild(
                $columnId,
                $this->getLayout()
                    ->createBlock('Plumrocket\Estimateddelivery\Block\Adminhtml\System\Config\Form\DatePeriod\InputTable\Column')
                    ->setData($column)
                    ->setId($columnId)
                    ->setGrid($this)
            );
