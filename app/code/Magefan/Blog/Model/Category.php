...
class Category extends \Magento\Framework\Model\AbstractModel {
    ...
    public function getParentCategory($storeFilter = false) {
        ...
        elseif ($pCategory = $this->getData($k)) {
            if (in_array(0, $this->getStoreId()) || in_array(0, $pCategory->getStoreId()) || array_intersect($this->getStoreId(), $pCategory->getStoreId())
            ) {
                return $pCategory;
            }
        }
    ...
    /**
     * @todo To get list tree option category.
     * @return string
     */
    public function getListOptionCategory($returnLableValue = TRUE) {
        $categories[] = ($returnLableValue) ? ['label' => 'All Categories', 'value' => 0] : 'Please select';
        ...
        foreach ($collection as $item) {
            if ($returnLableValue) {
                ...
                $categories[$item->getId()] = $this->_getSpaces($item->getLevel()) . ' ' . $item->getTitle() . ($item->getIsActive() ? '' : ' (' . __('Disabled') . ')' );                
            }
        }
        return $categories;
    }
