...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection
{
    ...
    public function __construct(
        ...
        $this->_hasRenderCategory = FALSE;
    }
    
    public function setRenderCategory($status = FALSE){
        $this->_hasRenderCategory = $status;
    }
    ...
    public function addFieldToFilter($field, $condition = null)
    {
        ...
        if ($field === 'category') {
            if(count($condition) == 1 && isset($condition["eq"]) && $condition["eq"] == 0){
                return FALSE;
            }
    ...
    protected function _afterLoad()
    {
        ...
        if (count($items)) {
            ...
            if($this->_hasRenderCategory){
//                var_dump($select);die;
                $select = $select->join( array('cate'=> $this->getTable('magefan_blog_category')), 
                                            'cate.category_id = cps.category_id', array('*'));
            }
            ...
            if ($result) {
                ...
                foreach($result as $item) {
                    ...
                    if($this->_hasRenderCategory){
                        $categories_name[$item['post_id']][] = $item['title'];
                    }
                    
                }

                foreach ($this as $item) {
//                    var_dump("<pre>");print_r($item->getData());die;
                    $postId = $item->getData('post_id');
                    if (isset($categories[$postId])) {
                        $item->setData('categories', $categories[$postId]);
                        if($this->_hasRenderCategory){
                            $item->setData('category_name', $categories_name[$postId]);
                        }
                    }
                }
