    ...
    $.widget('mage.amShopbyAjax',{
        ...
        initAjax: function()
        {
            ...
            $.mage.productListToolbarForm.prototype.changeUrl = function (paramName, paramValue, defaultValue) {
                ...
                if(this.options.order == paramName){
                    var orderParam = paramValue.split('_');
                    var attributeCode = orderParam[0];
                    var paramSort = orderParam[1];
                    if(orderParam.length > 2){
                        paramSort = orderParam.pop();
                        attributeCode = orderParam.join('_');
                    }
                    
                    paramData[paramName] = attributeCode;
                    paramData[this.options.direction] = paramSort;
                }else{            
