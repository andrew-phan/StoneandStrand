...
this.element.on('change',this.options.addrTypeSelector,$.proxy(this._handleShipAddrChange,this));},processFormDataArr:function(formDataArr){var formData=formDataArr.formData;for(var i=this.options.rowIndex=0;i<formData.length;this.options.rowIndex=i++){this.addRow(i);this._processFormDataArrKey(i,formData[i],false);}},_processFormDataArrKey:function(index,formRow,isCustom){var idPrefix=isCustom?this.options.rowCustomIdPrefix:this.options.rowIdPrefix;for(var key in formRow){if(formRow.hasOwnProperty(key)){if(Object.prototype.toString.call(formRow[key])==='[object Object]'){this._processFormDataArrKey(index,formRow[key],true);}else{this.setFieldById(idPrefix+key+index,formRow[key]);}}}},_handleShipAddrChange:function(e){$(this.options.newShipAddrFormSelector).toggle(e.target.value===this.options.newAddrTypeVal);}});return $.mage.giftRegistry;});