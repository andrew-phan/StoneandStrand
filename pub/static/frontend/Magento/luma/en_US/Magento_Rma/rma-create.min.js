...
this._recountRemQty();return false;}},_handleKeyup:function(e){var currElem=$(e.currentTarget),currElemId=currElem.attr('id'),currElemContainer=currElem.closest('fieldset').find('select[id^="'+this.options.itemsItem+'"]');if(currElemContainer.length<1){currElemContainer=currElem.closest('fieldset').siblings('fieldset').find('select[id^="'+this.options.itemsItem+'"]');}
var currElemIndex=currElemContainer.data('args').index;if(currElemId.match(/^items/)&&!e.handled){this._recountRemQty();var orderItemRemQty=0,orderItemReqQty=parseInt(currElem.val(),10);if(currElem.closest('tr').find('.remaining.qty').length>0){orderItemRemQty=parseInt(currElem.closest('tr').find('.remaining.qty').text(),10);}else{orderItemRemQty=parseInt($('[id^="remaining_quantity_'+currElemIndex+'"]').text(),10);}
if(orderItemRemQty<0){currElem.val(orderItemReqQty+orderItemRemQty);this._recountRemQty();}
e.handled=true;}},_esc:function(str){if(str){return str.replace(/([ ;&,.+*~\':"!\^$\[\]()=>|\/@])/g,'\\$1');}else{return str;}}});return $.mage.rmaCreate;});